...
public class PersistenceMetricsCommands implements ConsoleCommand {
    ...
    public boolean execute(UI console, Iterator<String> args) throws IllegalParametersException {
        ...
        try {
            ...
            generator.playerId(Long.valueOf(ppid)).timestamp(System.currentTimeMillis(), numberOfDays, TimeUnit.DAYS).count(numberOfFacts + startingFrom);
            int skipCount = 0;
            for (Gameplay g : generator.generate()) {
                if (skipCount++ > startingFrom) {
                    Key key = new GameplayKey(String.valueOf(g.getId()), "" + ppid);
                    cache.put(key, g);
                }
            }
