...
public class PutConsoleCommand implements ConsoleCommand {
    ...
    public boolean execute(UI console, Iterator<String> args) throws IllegalParametersException {
        try {
            ...
            String code = args.next();

            Gameplay g = new GameplayBuilder(player).gameCode(code).timestamp(System.currentTimeMillis()).build();
            g.setId(id);


            cache.put(new PlayerKey(id.toString(), player.toString()), g);

            console.println("Written (" + id + "," + g + ")");
        } catch (NumberFormatException e) {
            throw new IllegalParametersException("Expected usage: put <key> <player> <gamecode>\nValue for key has to be a number. Example:\n put 10 2 test");
    ...
    public void usage(UI console) {
        console.println(COMMAND_NAME + " <key> <player> <gamecode>");
        console.println("\t\tPut a fact (key:player, gamecode) in the Facts cache.");
    }
}
